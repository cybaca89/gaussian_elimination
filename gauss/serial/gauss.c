...
/* if n == 8000, then size of matrix is
 * 64,008,000 doubles, or
 * 512,064,000 bytes
 */

// extern void printm(const char *, double *, int, int);
// extern void print_result_values(double *, size_t);

// guassian elimination data
static struct {
    double

    /* heap data */
           *eche  // polynomial constant array
         , **rows  // original polynomial matrix array
         , *vars  // polynomial variable array
         , *orig  // original polynomial matrix array
...
static void _print_matrix(const char *, double *);
static void _generate_echelon(void);
static void back_substitution(void);
static void _l2_norm_validity_check(void);
static void _print_matrix_dp(void);
static double _pivot(const int beg, const int end);
